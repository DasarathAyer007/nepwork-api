...
class Payment(TimeStampedModel, SoftDeleteModel):
    ...
    # --- Reconciliation bookkeeping (added for the Celery verification worker) ---
    verification_attempts = models.PositiveIntegerField(
        default=0,
        help_text="Number of times the reconciliation worker has looked this payment up.",
    )
    last_verification_attempt_at = models.DateTimeField(
        null=True,
        blank=True,
    )
    next_verification_at = models.DateTimeField(
        null=True,
        blank=True,
        db_index=True,
        help_text="Earliest time the reconciliation worker should retry this payment.",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["status", "next_verification_at"],
                name="payment_reconciliation_idx",
            ),
        ]
    ...
    def clean(self):
        ...
        if self.provider == "khalti" and self.esewa_transaction_uuid:
            raise ValidationError(
                {
        ...
        if self.provider == "esewa" and self.pidx:
            raise ValidationError(
